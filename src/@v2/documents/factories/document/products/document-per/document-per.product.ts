...
import { IProductDocumentPER } from './document-per.types';

export class ProductDocumentPER implements IDocumentFactoryProduct<IProductDocumentPER, DocumentPGRModel> {
  public unlinkPaths: IUnlinkPaths[] = [];
  public type = 'Periculosidade';
  ...
  public async getAttachments({ data }: IGetAttachments<IProductDocumentPER, DocumentPGRModel>) {
    return [];
  }

  public async getSections({ data, attachments }: IGetDocument<IProductDocumentPER, DocumentPGRModel>) {
  ...
  public async save({ attachments, body, url }: ISaveDocument<IProductDocumentPER, DocumentPGRModel>) {
  ...
  public async error({ body }: ISaveErrorDocument<IProductDocumentPER>) {
    const documentVersion = new DocumentVersionEntity({
      id: body.documentVersionId,
  ...
  public getFileName = (data: DocumentPGRModel, type = 'PER') => {
  ...
  private async downloadImages(document: DocumentPGRModel) {
    ...
    const photos = document.homogeneousGroups
      .filter((group) => group.allRiskData.some((riskData) => riskData.risk.isPericulosidade))
