...
export const NewExamsByHierarchyHeader = (
  ...
  Array.from(hierarchyData.entries()).forEach(([, hierarchy]) => {
    hierarchy.org.forEach((org) => {
      if (!hierarchyType[org.typeEnum]) {
        ...
        hierarchyTypeArray[HierarchyPlanMap[org.typeEnum].position] = {
          text: HierarchyPlanMap[org.typeEnum].text,
          size: 3,
        };
      }
    });
