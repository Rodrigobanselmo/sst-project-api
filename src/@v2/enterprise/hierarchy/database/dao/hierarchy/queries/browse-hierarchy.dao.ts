...
export class HierarchyBrowseQuery {
  ...
  private browseWhere(filters: IHierarchyDAO.BrowseParams['filters']) {
    const where = [Prisma.sql`hierarchy."companyId" = ${filters.companyId}`, Prisma.sql`hierarchy."status"::text = ${StatusEnum.ACTIVE}`];
