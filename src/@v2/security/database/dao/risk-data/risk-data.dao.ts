...
import { IRiskDataDAO, RiskDataOrderByEnum } from './risk-data.types';
...
export class RiskDataDAO {
  ...
  async browse({ limit, page, orderBy, filters }: IRiskDataDAO.BrowseParams) {
  ...
  private browseWhere(filters: IRiskDataDAO.BrowseParams['filters']) {
    const where = [
  ...
  private filterWhere(filters: IRiskDataDAO.BrowseParams['filters']) {
  ...
  private browseOrderBy(orderBy?: IRiskDataDAO.BrowseParams['orderBy']) {
    if (!orderBy) return []
    ...
    const map: Record<RiskDataOrderByEnum, string> = {
      [RiskDataOrderByEnum.NAME]: 'cc.name',
      [RiskDataOrderByEnum.TYPE]: `CASE cc.type ${desiredOrder.map((type, index) => `WHEN '${type}' THEN ${index}`).join(' ')} ELSE ${desiredOrder.length} END`,
      [RiskDataOrderByEnum.CREATED_AT]: 'cc.created_at',
      [RiskDataOrderByEnum.UPDATED_AT]: 'cc.updated_at',
      [RiskDataOrderByEnum.DONE_AT]: 'cc.done_at',
      [RiskDataOrderByEnum.STAGE]: 'st.name',
      [RiskDataOrderByEnum.ORDER]: 'cc.order',
      [RiskDataOrderByEnum.PHOTOS]: 'total_photos',
      [RiskDataOrderByEnum.RISKS]: 'total_risks',
      [RiskDataOrderByEnum.HIERARCHY]: 'total_hierarchies',
      [RiskDataOrderByEnum.PROFILES]: 'total_profiles',
    }
