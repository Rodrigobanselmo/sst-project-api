...
export class DeleteFormPreliminaryLibraryQuestionUseCase {
  ...
  async execute(params: { companyId: string; questionId: string }) {
    ...
    if (blocksUsingQuestion > 0) {
      throw new BadRequestException('Esta pergunta não pode ser excluída porque ainda faz parte de um ou mais blocos ativos da biblioteca. Remova-a dos blocos antes de excluir.');
    }
