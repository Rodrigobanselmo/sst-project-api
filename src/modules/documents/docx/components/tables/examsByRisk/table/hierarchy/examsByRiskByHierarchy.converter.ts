...
export const examsByHierarchyConverter = ({ companyId, exams, hierarchyData, concatExamsAndRisks, withGroup, homoGroupTree, mergeCells }: IExamsByRiskByHierarchyData) => {
  ...
  data.forEach(({ id, ...hierarchy }) => {
    const examArray = Object.values(hierarchyExamsMap[id]).flat()
      .sort((b, a) => sortNumber(
        (a.origin.isAdmission ? 1 : 0) + (a.origin.isPeriodic ? 4 : 0) + (a.origin.isDismissal ? 7 : 0) + (a.origin.isChange ? 10 : 0) + (a.origin.isReturn ? 50 : 0),
        (b.origin.isAdmission ? 1 : 0) + (b.origin.isPeriodic ? 4 : 0) + (b.origin.isDismissal ? 7 : 0) + (b.origin.isChange ? 10 : 0) + (b.origin.isReturn ? 50 : 0),
      )).sort((a, b) => sortNumber(
        a.origin.validityInMonths || 0,
        b.origin.validityInMonths || 0,
      ))
    ...
    if (examArray.length && concatExamsAndRisks) {
      ...
      Object.values(examDataMap).forEach((examData) => {
        ...
        const text = isAll ? 'não vinculado a risco específico' : removeDuplicate(risks.map((risk) => getIsAll(risk?.id, risk?.name) ? '' : risk?.name).filter(Boolean), { simpleCompare: true }).join('\n');

        cells[ExamByHierarchyColumnEnum.RISKS] = { text, size: 5, ...(isAll && { color: palette.text.simple.string }), fontSize: 10 };

        const exams = removeDuplicate(examData.map((examOrigin) => examOrigin.name), { simpleCompare: true }).join('\n');

        cells[ExamByHierarchyColumnEnum.EXAMS] = { text: exams, size: 5, fontSize: 10 };
        cells[ExamByHierarchyColumnEnum.PERIODICIDADE] = { text: examData[0]?.origin.isPeriodic ? String(examData[0].origin.validityInMonths || '-') : '', size: 1 };
    ...
    examArray.forEach((examOrigin) => {
      ...
      if (withGroup) {
        const homoGroupsNames: string[] = []
        hierarchy.allHomogeneousGroupIds.forEach(id => {
          if (homoGroupTree[id]?.type !== HomoTypeEnum.HIERARCHY) homoGroupsNames.push(getHomoGroupName(homoGroupTree[id]).nameOrigin)
        })
