...
export const riskCharacterizationConverter = (riskGroup: RiskFactorGroupDataEntity) => {
  ...
  riskGroup.data.forEach((riskData) => {
    ...
    cells[RiskCharacterizationColumnEnum.AGENT] = { text: risk.name || '--', size: 4, type: risk.type, shading: { fill: palette.table.header.string } }
    cells[RiskCharacterizationColumnEnum.CAS] = { text: risk.cas || '--', size: 2 }
    cells[RiskCharacterizationColumnEnum.PROPAGATION] = { text: (risk.propagation||[]).join('\n') || '--' , size: 2 }
    cells[RiskCharacterizationColumnEnum.UNIT] = { text: risk.unit || '--', size: 2 }
    cells[RiskCharacterizationColumnEnum.NR15LT] = { text: risk.nr15lt || '--', size: 2 }
    cells[RiskCharacterizationColumnEnum.ACGIH_TWA] = { text: risk.twa || '--', size: 3 }
    cells[RiskCharacterizationColumnEnum.ACGIH_STEL] = { text: risk.stel || '--', size: 2 }
    cells[RiskCharacterizationColumnEnum.IPVS] = { text: risk.ipvs || '--', size: 2 }
    cells[RiskCharacterizationColumnEnum.PV] = { text: risk.pv || '--', size: 2 }
    cells[RiskCharacterizationColumnEnum.PE] = { text: risk.pe || '--', size: 2 }
    cells[RiskCharacterizationColumnEnum.CARNOGENICITY_ACGIH] = { text: risk.carnogenicityACGIH || '--', size: 2 }
    cells[RiskCharacterizationColumnEnum.CARNOGENICITY_LINACH] = { text: risk.carnogenicityLinach || '--', size: 2 }
    cells[RiskCharacterizationColumnEnum.BEI] = { text: risk.exame || '--', size: 2 }
