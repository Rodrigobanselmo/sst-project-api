...
export abstract class ReportFactoryAbstractionCreator<T> {
  ...
  public async excelCompile(companyId: string, query: T): Promise<{ workbook: Workbook; filename: string }> {
    ...
    const { workbook, filename } = await this.excelProvider.createReportTable(
      sheets.map(({ rows, sheetName }) => ({ rows, name: sheetName })),
      sheets[0].filename,
    );
  ...
  public async getRows(companyId: string, query: T) {
    ...
    const data = await asyncEach(products, async (product, index) => {
      ...
      const company = await this.companyRepo.findFirstNude({
        where: { id: companyId },
        select: { id: true, name: true, fantasy: true },
      });
  ...
  public organizeRows(findArgs: IReportFactoryProductFindData): IReportRows {
    ...
    rows.unshift(
      ...findArgs.titleRows.map<IReportCell[]>((row) =>
        row.map<IReportCell>((t) => ({ fill: ReportFillColorEnum.TITLE, ...t })),
      ),
    );
    rows.push(
      ...findArgs.endRows.map<IReportCell[]>((row) =>
        row.map<IReportCell>((t) => ({ fill: ReportFillColorEnum.END, ...t })),
      ),
    );
