...
import {
  AppendixEnumMap,
  OtherAppendixEnum,
  OtherAppendixEnumMap,
} from './../../../../../../shared/constants/enum/appendix';
...
import { IReportCell, IReportHeader, IReportSanitizeData } from '../../types/IReportFactory.types';
...
export function getConcatRisksData(data: PromiseInfer<ReturnType<DocumentPGRFactoryProduct['getPrgRiskData']>>) {
  ...
  data.riskGroupData?.data?.forEach((riskData) => {
    ...
    map[riskId].generateSources = removeDuplicate([...map[riskId].generateSources, ...riskData.generateSources], {
      removeById: 'id',
    });
    map[riskId].recs = removeDuplicate([...map[riskId].recs, ...riskData.recs], { removeById: 'id' });
    map[riskId].engs = removeDuplicate([...map[riskId].engs, ...riskData.engs], { removeById: 'id' }).filter(
      (e) => !isNaRecMed(e.medName),
    );
    map[riskId].adms = removeDuplicate([...map[riskId].adms, ...riskData.adms], { removeById: 'id' }).filter(
      (e) => !isNaRecMed(e.medName),
    );
    map[riskId].epis = removeDuplicate([...map[riskId].epis, ...riskData.epis], { removeById: 'id' }).filter(
      (e) => !isNaEpi(e.ca),
    );
...
export class RiskStructureRsDataNR extends ReportRiskStructureProduct {
  ...
  public sanitizeData(
    ...
    params: DownloadRiskStructureReportDto,
  ): IReportSanitizeData[] {
    const rows: IReportSanitizeData[] = [];
    const errors: string[] = [];

    Array.from(data.hierarchyData.values()).forEach((hierarchy) => {
      return this.getRiskDataByHierarchy({ hierarchy, riskData: getConcatRisksData(data) })
        ...
        .map((riskData) => {
          ...
          const engs = riskData.engs
            .map((gs) => gs.medName)
            .filter(Boolean)
            .join('; ');
          const adms = riskData.adms
            .map((gs) => gs.medName)
            .filter(Boolean)
            .join('; ');
          const recs = riskData.recs
            .map((gs) => gs.recName)
            .filter(Boolean)
            .join('; ');
          const meds = [engs, adms].filter(Boolean).join('; ');
          const gs = riskData.generateSources
            .map((gs) => gs.name)
            .filter(Boolean)
            .join('; ');
          ...
          if (otherappendix) {
            activitiesJson?.activities?.forEach((activity) => {
              if (activity.description)
                rowsData.push({ value: otherappendix, activity: [activity.description, activity.subActivity] });
            });
          }
  ...
  public getTitle(_: IReportHeader, company: CompanyEntity): IReportCell[][] {
    ...
    const version: IReportCell[] = [
      { content: 'Versão', font: { size: 12, bold: true, color: { theme: 1 }, name: 'Calibri' } },
      { content: '2023.1', font: { size: 12, bold: true, color: { theme: 1 }, name: 'Calibri' } },
    ];
    const versionValue: IReportCell[] = [
      { content: 1, font: { size: 12, bold: true, color: { theme: 1 }, name: 'Calibri' } },
    ];
