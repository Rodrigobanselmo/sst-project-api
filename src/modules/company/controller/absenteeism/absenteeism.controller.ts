...
import {
  CreateAbsenteeismDto,
  FindAbsenteeismDto,
  UpdateAbsenteeismDto,
} from "../../dto/absenteeism.dto";
...
export class AbsenteeismController {
  ...
  find(
    @User() userPayloadDto: UserPayloadDto,
    @Query() query: FindAbsenteeismDto,
  ) {
  ...
  @Post("/:companyId")
  create(
    @Body() upsertAccessGroupDto: CreateAbsenteeismDto,
    @User() userPayloadDto: UserPayloadDto,
  ) {
    return this.createAbsenteeismsService.execute(
  ...
  update(
    ...
    return this.updateAbsenteeismsService.execute(
