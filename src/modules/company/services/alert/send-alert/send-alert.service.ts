...
export class SendAlertService {
      ...
      include: {
        groups: {
          select: {
            ...
            id: true,
            companyId: true,
            users: { where: { companyId: companyId, status: 'ACTIVE' }, select: { user: { select: { email: true } } } },
          },
        },
      },
    });
  ...
  async updateNextAlertAlert(type: AlertsTypeEnum, companyId: string, alert: AlertEntity) {
    if (alert.defaultNextAlert) await this.alertRepository.upsert({ companyId, type, nextAlert: this.upsertAlertService.getNextDate(alert.configJson) });
  }
  ...
  async getTemplateExamExpired(companyId: string) {
    ...
    const expiredCount = await this.employeeRepository.countNude({
      where: { companyId: companyId, OR: [{ expiredDateExam: { lte: new Date() } }, { expiredDateExam: null }] },
    });
    const expiringSoonCount = await this.employeeRepository.countNude({
      where: { companyId: companyId, AND: [{ expiredDateExam: { gt: new Date() } }, { expiredDateExam: { lte: closeDate } }] },
    });
    ...
    const path = resolve(__dirname, '..', '..', '..', '..', '..', '..', 'templates', 'email', 'alerts', 'expiredExamAlert.hbs');
    const variables = {
