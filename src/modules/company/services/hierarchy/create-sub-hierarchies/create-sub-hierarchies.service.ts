...
export class CreateSubHierarchyService {
  ...
  async execute(hierarchy: CreateSubHierarchyDto, user: UserPayloadDto) {
    ...
    if (!isEveryoneFromSameOffice && !hierarchy.organizeByOffice)
      throw new BadRequestException(ErrorCompanyEnum.EVERYONE_NOT_FROM_SAME_OFFICE);

    const officesMap = employees.reduce(
      (acc, employee) => {
