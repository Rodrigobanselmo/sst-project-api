...
export class DeleteCompanyService {
  ...
  async execute(user: UserPayloadDto, option?: { clinic?: boolean }) {
    ...
    await this.prisma.companyCharacterization.deleteMany({ where: { OR: [{ companyId: id }, { homogeneousGroup: { companyId: id } }] } });
    await this.prisma.companyEnvironment.deleteMany({ where: { OR: [{ companyId: id }, { homogeneousGroup: { companyId: id } }] } });
