...
let UploadEmployeesService = class UploadEmployeesService {
    ...
    async execute(file, userPayloadDto) {
        ...
        employeesData = employeesData.map((employee) => {
            const workplace = workplaces.find((work) => work.abbreviation === employee.abbreviation);
            if (!workplace)
                throw new common_1.BadRequestException(errorMessage_1.ErrorCompanyEnum.WORKPLACE_NOT_FOUND);
            delete employee.abbreviation;
            return Object.assign(Object.assign({}, employee), { workplaceId: workplace.id });
        });
