...
let FilesChecklistController = class FilesChecklistController {
    constructor(uploadEpiDataService, uploadChecklistDataService, downloadRiskDataService) {
        this.uploadEpiDataService = uploadEpiDataService;
        this.uploadChecklistDataService = uploadChecklistDataService;
        this.downloadRiskDataService = downloadRiskDataService;
    }
    async uploadRiskFile(file, userPayloadDto, res) {
        const { workbook, filename } = await this.uploadChecklistDataService.execute(file, userPayloadDto);
    ...
    async downloadRisks(userPayloadDto, res) {
        const { workbook, filename } = await this.downloadRiskDataService.execute(userPayloadDto);
        res.attachment(filename);
        workbook.xlsx.write(res).then(function () {
            res.end();
        });
