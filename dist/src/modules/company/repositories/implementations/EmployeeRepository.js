...
let EmployeeRepository = class EmployeeRepository {
    ...
    async create(_a) {
        var { workspaceIds, hierarchyId, companyId } = _a, createCompanyDto = __rest(_a, ["workspaceIds", "hierarchyId", "companyId"]);
        const employee = await this.prisma.employee.create({
            data: Object.assign(Object.assign({}, createCompanyDto), { company: { connect: { id: companyId } }, workspaces: {
    ...
    async update(_a) {
        var { workspaceIds, hierarchyId, companyId, id } = _a, createCompanyDto = __rest(_a, ["workspaceIds", "hierarchyId", "companyId", "id"]);
        const employee = await this.prisma.employee.update({
            data: Object.assign(Object.assign({}, createCompanyDto), { workspaces: !workspaceIds
                    ? undefined
                    : {
    ...
    async upsertMany(upsertEmployeeMany, companyId) {
        const data = await this.prisma.$transaction(upsertEmployeeMany.map((_a) => {
            var { companyId: _, id, workspaceIds, hierarchyId } = _a, upsertEmployeeDto = __rest(_a, ["companyId", "id", "workspaceIds", "hierarchyId"]);
            return this.prisma.employee.upsert({
                create: Object.assign(Object.assign({}, upsertEmployeeDto), { company: { connect: { id: companyId } }, workspaces: {
                        connect: workspaceIds.map((id) => ({
                            id_companyId: { companyId, id },
                        })),
                    }, hierarchy: {
                ...
                update: Object.assign(Object.assign({}, upsertEmployeeDto), { workspaces: !workspaceIds
                        ? undefined
                        : {
                            set: workspaceIds.map((id) => ({
                                id_companyId: { companyId, id },
