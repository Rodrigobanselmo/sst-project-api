...
let HomoGroupRepository = class HomoGroupRepository {
    ...
    async create(_a, companyId) {
        var { hierarchies } = _a, updateHomoGroup = __rest(_a, ["hierarchies"]);
        const data = await this.prisma.homogeneousGroup.create({
            data: Object.assign(Object.assign({}, updateHomoGroup), { companyId }),
        });
        ...
        return new homoGroup_entity_1.HomoGroupEntity(data);
    }
    async update(_a) {
        ...
        const data = await this.prisma.homogeneousGroup.update({
            ...
            data: Object.assign(Object.assign({}, updateHomoGroup), { hierarchies: hierarchies
                    ? { set: hierarchies.map((id) => ({ id })) }
                    : undefined }),
        });
        return new homoGroup_entity_1.HomoGroupEntity(data);
    ...
    async findHomoGroupByCompany(companyId) {
        ...
        return hierarchies.map((homoGroup) => new homoGroup_entity_1.HomoGroupEntity(homoGroup));
    }
};
