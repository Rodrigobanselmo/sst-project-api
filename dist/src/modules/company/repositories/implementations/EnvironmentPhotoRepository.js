...
let EnvironmentPhotoRepository = class EnvironmentPhotoRepository {
    ...
    async update(_a) {
        var { id } = _a, environmentPhotoDto = __rest(_a, ["id"]);
        const environment = await this.prisma.companyCharacterizationPhoto.update({
            where: { id: id || 'no-id' },
            data: Object.assign({}, environmentPhotoDto),
        });
    ...
    async findByEnvironment(environmentId) {
        ...
        return environments.map((environment) => new environment_photo_entity_1.EnvironmentPhotoEntity(environment));
    }
    async delete(id) {
