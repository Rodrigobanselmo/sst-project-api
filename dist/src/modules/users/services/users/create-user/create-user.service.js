...
let CreateUserService = class CreateUserService {
    constructor(userRepository, findByTokenService, dateProvider, hashProvider, inviteUsersRepository) {
    ...
    async execute(_a) {
        ...
        const { companies, email, companyId } = await (0, exports.getCompanyPermissionByToken)(token, restCreateUserDto.email, this.findByTokenService, this.dateProvider);
        ...
        if (email && companyId)
            await this.inviteUsersRepository.deleteByCompanyIdAndEmail(companyId, email);
        return user;
    }
};
...
const getCompanyPermissionByToken = async (token, email, findByTokenService, dateProvider) => {
    ...
    if (invite.email !== email)
        throw new common_1.BadRequestException(errorMessage_1.ErrorInvitesEnum.TOKEN_NOT_VALID_EMAIL);
