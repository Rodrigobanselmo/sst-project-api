...
let UpsertExamToClinicService = class UpsertExamToClinicService {
    ...
    async execute(createExamDto, user) {
        ...
        if (clinicExamActual && clinicExamActual.startDate > createExamDto.startDate)
            throw new common_1.BadRequestException('A data de início deve ser maior que a data de início anterior');
        if (clinicExamActual)
            await this.examToClinicRepository.update({
                examId: createExamDto.examId,
                companyId: user.targetCompanyId,
        ...
        const ExamFactor = await this.examToClinicRepository.create(Object.assign(Object.assign({}, createExamDto), { companyId: user.targetCompanyId }));
        return ExamFactor;
    }
};
...
//# sourceMappingURL=upsert-exam-to-clinic-copy.js.map