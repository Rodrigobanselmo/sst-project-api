...
let PrismaDbExceptionFilter = class PrismaDbExceptionFilter {
        ...
        let error = new common_1.HttpException(errorMessage_1.ErrorMessageEnum.PRISMA_ERROR, 500);
        const { cause, target, field_name } = meta;
        console.log(exception);
        switch (code) {
            case 'P2002':
                if (target)
                    error = new common_1.BadRequestException(`Dado que está tentando criar já existe: ${target.join(', ')} está em conflito`);
                break;
            case 'P2003':
                if (field_name)
                    error = new common_1.BadRequestException(`está faltando campos para realizar essa operação: ${field_name}`);
                break;
            case 'P2025':
                if (cause)
        ...
        response.status(status).json({
            ...
            message: error.message || errorMessage_1.ErrorMessageEnum.PRISMA_ERROR,
            error: error.name,
            path: request.url,
        });
    }
