...
const seedRisks = async (prisma, companyId) => {
    const ids = [
    ...
    await Promise.all(ids.map(async (_id) => await prisma.recMed.createMany({
        data: [
            {
                id: (0, uuid_1.v4)(),
                companyId: companyId,
    ...
    await Promise.all(ids.map(async (id) => {
        await Promise.all(recMed.map(async (rec) => await prisma.recMed.createMany({
            data: [
                {
                    id: (0, uuid_1.v4)(),
                    companyId: companyId,
